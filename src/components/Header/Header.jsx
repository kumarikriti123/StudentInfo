    ...
    <>
      <div className="flex justify-between items-center px-5  fixed top-0 left-0 z-50 bg-white w-full  pt-10 pb-5 ">
        <div className="flex items-center z-50">
          ...
          <div className="font-Merriweather font-bold text-xl ml-2 mt-2 text-red-900  md:hidden lg:block" >
            CodingAge
          </div>
        </div>
        ...
        <div className="hidden md:flex md:h-14 xl:h-11 xl:mx-40 md:mx-5 lg:mx-16 bg-gradient-to-r from-red-800 to-red-600 xl:py-2 xl:px-8 lg:py-1 lg:px-4 md:px-2 md:py-1 rounded-2xl  font-bold font-Rubik text-white">
      ...
      {isSidebarOpen && (
        <div className="fixed pt-[12vh] left-0 w-64 h-full bg-white z-20 shadow-lg md:hidden sm:w-80">
          <div className="flex flex-col p-5">
            {["HOME", "BATCHES", "STUDENTS", "DASHBOARD", "ABOUT", "CONTACT US"].map((name) => (
              <button
                key={name}
                className={`${headerName === name ? 'bg-gradient-to-r from-red-800 to-red-600 text-white ' : ''}mb-3 py-2 px-4 rounded-2xl font-bold font-Rubik text-red-900 hover:bg-red-600 hover:text-white`}
                onClick={() => handleMenuItemClick(name)}
              >
                {name}
              </button>
